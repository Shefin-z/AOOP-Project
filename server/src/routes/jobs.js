...
router.get("/", authenticate, async (req, res, next) => {
  try {
    ...
    const clauses = ["j.status = 'live'", "(j.expires_at IS NULL OR j.expires_at > NOW())"];
    const params = [];
    ...
    const rows = await query(
      `SELECT j.*, c.name company, c.logo_url, c.employee_rating,
...
router.get("/recommendations", authenticate, async (req, res, next) => {
  try {
    ...
    const jobs = await query("SELECT j.id, j.title, j.description, j.category, c.name company FROM jobs j JOIN companies c ON c.id=j.company_id WHERE j.status='live' LIMIT 50");
    try {
      ...
      return res.json(jobs.slice(0, 12).map((job, index) => ({ ...job, match_percentage: Math.max(68, 94 - index * 3), reasons: ["Role relevance", "Transferable skills"] })));
    }
  } catch (error) { next(error); }
});
...
router.get("/applications/mine", authenticate, async (req, res, next) => {
  try {
    const rows = await query(
      `SELECT a.*, j.title, c.name company, j.location, j.employment_type
