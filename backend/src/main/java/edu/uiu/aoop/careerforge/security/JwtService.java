...
public class JwtService {
  ...
  public AppPrincipal parse(String token) { var claims=Jwts.parser().verifyWith((javax.crypto.SecretKey) key()).build().parseSignedClaims(token).getPayload(); return new AppPrincipal(Long.valueOf(claims.getSubject()), claims.getSubject(), edu.uiu.aoop.careerforge.model.Role.valueOf(claims.get("role",String.class)), claims.get("name",String.class)); }
}
