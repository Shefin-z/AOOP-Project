...
public class CareerProgressController {
  ...
  @PostMapping("/vault/documents") @Transactional Map<String,Object> addDocument(@AuthenticationPrincipal AppPrincipal principal,@RequestBody Map<String,Object> body) { student(principal); String title=text(body,"title",180);String fileName=text(body,"fileName",255);String contentType=text(body,"contentType",100);String storagePath=text(body,"storagePath",500);if(title.isBlank()||fileName.isBlank()||storagePath.isBlank())throw new ApiException(HttpStatus.BAD_REQUEST,"Title, file name and storage path are required");jdbc.update("INSERT INTO student_documents (user_id,title,file_name,content_type,storage_path) VALUES (?,?,?,?,?)",principal.id(),title,fileName,contentType,storagePath);return Map.of("id",jdbc.queryForObject("SELECT LAST_INSERT_ID()",Long.class),"message","Document metadata saved"); }
