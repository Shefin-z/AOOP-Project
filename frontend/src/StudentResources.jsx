...
export function StudentResources() {
  ...
  return <section className="student-resources">
    ...
    <section className="skill-resource-finder">
      <p className="eyebrow"><Youtube size={15} /> SKILL RESOURCE FINDER</p><h3>What skill do you want to gain?</h3><p>Search a skill for YouTube's top public video results. A matching resource selected by your CareerForge administrator always appears first.</p>
      <form onSubmit={findSkill}><label><Search size={20} /><input value={skill} onChange={(event) => setSkill(event.target.value)} placeholder="e.g. React, SQL, Figma, Python, public speaking" /></label><button disabled={searching || !skill.trim()} type="submit">Search YouTube</button></form>
      ...
      {searching ? <div className="skill-searching" role="status"><LoaderCircle size={37} /><b>Finding YouTube's top results for {skill}...</b><span>Checking CareerForge recommendations first, then YouTube's public relevance results.</span></div> : !searchResults ? <div className="skill-empty"><Youtube size={27} /><b>Search the skill you want to learn</b><span>CareerForge will show matching administrator suggestions before YouTube's top public video results.</span></div> : <div className="skill-results">
        {(searchResults.suggestions?.length > 0 || searchResults.videos?.length > 0) && <><div className="skill-result-heading"><BadgeCheck size={17} /><div><b>{searchResults.suggestions?.length > 0 ? "CareerForge pick and top YouTube videos" : "Top YouTube video results"}</b><span>{searchResults.suggestions?.length > 0 ? "Your administrator's matching pick is first; YouTube's top public videos follow." : `${searchResults.videos.length} top YouTube video${searchResults.videos.length === 1 ? "" : "s"} for ${skill}`}</span></div></div><div className="resource-grid skill-result-grid">{searchResults.suggestions?.map((item) => <ResourceCard key={item.id} resource={item} onToggle={toggle} updatingId={updatingId} />)}{searchResults.videos?.map((item) => <article className="resource-card video-result" key={item.id}><div className="resource-thumbnail">{item.thumbnailUrl ? <img src={item.thumbnailUrl} alt="" /> : <div className="resource-thumbnail-fallback youtube"><Youtube size={32} /></div>}<span className="resource-youtube-badge"><Youtube size={13} /> YouTube video</span></div><div className="resource-card-top"><span className="resource-provider">{item.providerName}</span></div><h4>{item.title}</h4><p className="resource-description">{item.description || "A top YouTube video for this skill."}</p><footer><span>Video</span><a href={item.resourceUrl} target="_blank" rel="noreferrer">Watch on YouTube <ExternalLink size={14} /></a></footer></article>)}</div></>}
