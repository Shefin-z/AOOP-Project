      ...
      <div className="vault-layout">
        <div className="vault-editor">
          <form className="data-form" onSubmit={saveResume}>
            ...
            <label>Version name<input value={title} onChange={(event) => setTitle(event.target.value)} required placeholder="Software Engineer CV" /></label>
            <div className="form-grid">
              ...
              <label>Phone<input name="phone" value={content.phone} onChange={change} placeholder="01XXXXXXXXX" /></label>
              <label>Location<input name="location" value={content.location} onChange={change} placeholder="Dhaka, Bangladesh" /></label>
              <label className="full-field">Professional headline<input name="headline" value={content.headline} onChange={change} placeholder="Junior Software Engineer" /></label>
            </div>
            ...
            <div className="vault-item-builder full-field">
              ...
              <div className="vault-item-composer">
                <input value={skillDraft} onChange={(event) => setSkillDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addSkill(); } }} placeholder="e.g. Java, Spring Boot, MySQL" />
            ...
            <label className="full-field">Education <small>(one item per line)</small><textarea name="education" rows="3" value={content.education} onChange={change} placeholder="BSc in CSE — United International University" /></label>
            <label className="full-field">Experience <small>(one item per line)</small><textarea name="experience" rows="3" value={content.experience} onChange={change} placeholder="Intern — Company, 2026" /></label>
            <div className="vault-item-builder full-field">
              ...
              <div className="vault-project-composer">
                <input value={projectDraft.title} onChange={(event) => setProjectDraft((draft) => ({ ...draft, title: event.target.value }))} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addProject(); } }} placeholder="Project name" />
                <input value={projectDraft.description} onChange={(event) => setProjectDraft((draft) => ({ ...draft, description: event.target.value }))} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addProject(); } }} placeholder="What did you build?" />
...
function AdminJobs() {
  ...
  const field = (label, name, type = "text", placeholder = "") => <label>{label}<input name={name} type={type} value={job[name] ?? ""} onChange={change} placeholder={placeholder} required={["title", "companyName", "expiryDate"].includes(name)} /></label>;
