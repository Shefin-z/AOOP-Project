...
public class CommunityFeedService {
    ...
    private final PostLikeStore likes; private final PostShareStore shares; private final ContentReportStore reports; private final CommunityModerationService moderation;
    public CommunityFeedService(AccessService access, UserRepository users, CommunityPostRepository posts, CommunityCommentRepository comments, PostLikeStore likes, PostShareStore shares, ContentReportStore reports, CommunityModerationService moderation) { this.access = access; this.users = users; this.posts = posts; this.comments = comments; this.likes = likes; this.shares = shares; this.reports = reports; this.moderation = moderation; }
    ...
    @Transactional public PostResponse create(Long userId, String content, String mediaUrl, String topicTags) { access.requireStudent(userId); LocalDateTime now = LocalDateTime.now(); posts.findTopByUserIdOrderByCreatedAtDesc(userId).filter(post -> post.getCreatedAt() != null && post.getCreatedAt().isAfter(now.minusHours(12))).ifPresent(post -> { throw new ResponseStatusException(HttpStatus.TOO_MANY_REQUESTS, "You can create another community post after the 12-hour cooldown."); }); String cleaned = clean(content, 5000); CommunityModerationService.Result result = moderation.analyse(cleaned, posts.findByUserIdAndCreatedAtAfter(userId, now.minusHours(24))); CommunityPost post = new CommunityPost(userId, cleaned, optional(mediaUrl, 500), optional(topicTags, 500), result.spamScore(), result.fraudScore(), result.riskScore(), result.riskLabel(), result.reasons(), result.status()); return response(posts.save(post), userId); }
