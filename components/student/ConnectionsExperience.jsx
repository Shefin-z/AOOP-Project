    ...
    <div className="space-y-5">
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(320px,.86fr)_minmax(0,1.35fr)]">
        ...
        <section className="panel flex min-h-[620px] flex-col overflow-hidden p-0">
          ...
          {selectedConnection && <><header className="flex items-center gap-3 border-b border-ink/[0.07] bg-white/45 px-5 py-4 dark:bg-white/[0.025]"><Avatar student={selectedConnection} size="h-11 w-11" /><div className="min-w-0 flex-1"><h2 className="truncate font-extrabold">{selectedConnection.name}</h2><p className="truncate text-xs text-muted">{profileLine(selectedConnection)} · Student ID {selectedConnection.student_id}</p></div><button onClick={removeConnection} className="btn-ghost text-xs text-muted hover:text-coral">Remove</button></header><div className="flex-1 space-y-3 overflow-y-auto bg-canvas/45 px-5 py-5">{messagesLoading && <p className="flex items-center justify-center gap-2 pt-12 text-xs text-muted"><LoaderCircle size={16} className="animate-spin" /> Loading conversation...</p>}{messageError && <div className="mx-auto max-w-md rounded-2xl bg-coral/10 p-4 text-center text-xs text-coral"><AlertTriangle className="mx-auto mb-2" size={17} />{messageError}<button onClick={() => loadMessages(selectedConnection.connection_id)} className="mt-2 block w-full font-extrabold underline">Try again</button></div>}{!messagesLoading && !messageError && !messages.length && <div className="mx-auto max-w-sm pt-20 text-center"><MessageCircle className="mx-auto text-cobalt/60" size={25} /><p className="mt-3 text-sm font-bold">Say hello to {selectedConnection.name.split(" ")[0]}.</p><p className="mt-1 text-xs text-muted">Your messages are private to this connection.</p></div>}{messages.map((message) => { const mine = Number(message.sender_id) === Number(currentUser.id); return <div key={message.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}><div className={`max-w-[82%] rounded-[18px] px-4 py-3 text-sm shadow-sm ${mine ? "rounded-br-md bg-cobalt text-white" : "rounded-bl-md bg-[#EEF2FF] text-[#172033] dark:bg-[#293044] dark:text-white"}`}><p className="whitespace-pre-wrap break-words leading-6">{message.body}</p><small className={`mt-1.5 block text-[10px] ${mine ? "text-white/60" : "text-[#566176] dark:text-white/70"}`}>{displayTime(message.created_at, true)}{mine && message.read_at ? " · Seen" : ""}</small></div></div>; })}<div ref={messageEndRef} /></div><form onSubmit={sendMessage} className="border-t border-ink/[0.07] bg-white/45 p-4 dark:bg-white/[0.025]"><div className="flex items-end gap-3"><textarea value={draft} onChange={(event) => setDraft(event.target.value)} className="input min-h-11 max-h-32 flex-1 resize-y py-2.5" maxLength={2000} placeholder={`Message ${selectedConnection.name.split(" ")[0]}...`} aria-label="Write a message" /><button disabled={!draft.trim() || sending} className="btn-accent min-h-11 px-4 disabled:opacity-45" aria-label="Send message">{sending ? <LoaderCircle className="animate-spin" size={16} /> : <Send size={16} />}</button></div><p className="mt-2 text-[10px] text-muted">Press Enter in this box to send a line break, or use the send button.</p></form></>}
        </section>
      </section>
    </div>
  );
